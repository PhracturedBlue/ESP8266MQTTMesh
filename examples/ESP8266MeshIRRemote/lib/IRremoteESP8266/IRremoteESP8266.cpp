...
//+=============================================================================
//
bool IRsend::sendPronto (const char* s,  bool repeat,  bool fallback)
{
	int       i;
    int       hex_count = 0;
	int       len;
	...
	for (cp = s;  *cp;  cp += 4) {
		...
		if ( !ishex(cp[0]) || !ishex(cp[1]) ||
		     !ishex(cp[2]) || !ishex(cp[3]))
        {
            Serial.println("Error at:" + String(cp - s));
            for (int i = 0; i < 4; i++) {Serial.print(cp[i], HEX); Serial.print(" ");} Serial.println("");
            return false ;
        }
        hex_count++;
	}
	...
	byp(&cp);
	if (htow(cp) != 0000)  {
        Serial.print("Error Bad Header: ");
        Serial.println(htow(cp), HEX);
        return false;
    }
	cp += 4;
	...
	rpt = htow(cp);
	cp += 4;
    if (hex_count != 4 + (2 * (once+rpt))) {
        Serial.println("Got unexpected data length: " + String(hex_count) + " != 4 + " + String(2*once) + " + " + String(2*rpt));
        return false;
    }
	...
	if (fallback) { // fallback on the "other" code if "this" code is not present
		if (!repeat) { // requested 'once'
			...
			if (rpt)   len = rpt  * 2,  skip = once *2;  // if rpt exists send it
		...
		else          len = rpt  * 2,  skip = once *2;  // 'repeat' starts where 'once' ends
    }
    //Serial.print("Skip: " + String(skip));
    //Serial.println(" Len: " + String(len));
    //Serial.println("Hex count: " + String(hex_count));
